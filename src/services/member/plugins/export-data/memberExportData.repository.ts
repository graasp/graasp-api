...
import type {
  ...
  ItemLikeWithItem,
  ItemMembershipRaw,
  LearningGoalCompletionRaw,
  LearningWorkspaceRaw,
} from '../../../../drizzle/types';
...
export class ExportDataRepository {
  ...
  /**
   * Return the private learning workspaces owned by the given account.
   */
  async getLearningWorkspaces(
    dbConnection: DBConnection,
    accountId: string,
    ...
    return dbConnection.query.learningWorkspacesTable.findMany({
      where: eq(learningWorkspacesTable.accountId, accountId),
      orderBy: desc(learningWorkspacesTable.updatedAt),
    });
  }

  /**
   * Return only the private learning goal completions owned by the given account.
   */
  async getLearningGoalCompletions(
    dbConnection: DBConnection,
    accountId: string,
  ): Promise<
    Array<
      Pick<LearningGoalCompletionRaw, 'goalId' | 'accountId' | 'completedAt'> & {
        itemId: string;
      }
    >
  > {
    ...
    return dbConnection
      .select({
        ...
        itemId: learningGoalsTable.itemId,
        accountId: learningGoalCompletionsTable.accountId,
      ...
      .where(eq(learningGoalCompletionsTable.accountId, accountId))
      .orderBy(desc(learningGoalCompletionsTable.completedAt));
  }
